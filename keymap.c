...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [1] = LAYOUT( /* Layer Zero */
	...
	KC_CAPS, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT,          KC_ENT,
	LGUI(LSFT(KC_S)), KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT,          KC_PAUSE, KC_UP,
	KC_LCTL, KC_LGUI, KC_LALT,                           KC_SPC,                                       KC_RALT, KC_RCTL, KC_LEFT,  KC_DOWN, KC_RGHT
  ),
    ...
    switch (keycode) {
        case KC_LALT:
            if (record->event.pressed) {
                layer_on(1);  // L_ALT 키를 누르는 동안 _LAYER1 레이어를 활성화
            } else {
                layer_off(1); // L_ALT 키를 뗄 때 _LAYER1 레이어를 비활성화
            }
            return false;
        default:
            return true;
